...
class PNIPacking(Document):
    ...
    def update_packing(self):
        ...
        packing.status = "Completed"
        packing.total_shift_stock = float(
            packing.total_stock) + float(packing.loose_stock) - float(packing.last_shift_loose_stock)
        packing.save()
        packing.submit()
        frappe.db.set(packing, 'status', 'Completed')
    ...
    def set_machine_helper(self):
        helper_id = helper = ""
        if self.shift == "Day":
            helper = frappe.get_value(
                "Workstation", self.workstation, "workstation_helper_name")
            helper_id = frappe.get_value(
                "Workstation", self.workstation, "workstation_helper")
        else:
            helper = frappe.get_value(
                "Workstation", self.workstation, "workstation_helper_name_night_shift")
            helper_id = frappe.get_value(
                "Workstation", self.workstation, "workstation_helper_night_shift")
    ...
    def on_submit(self):
        if self.pni_packing:
            self.update_packing()
        if not self.employee and not self.loose_entry:
    ...
    def get_employee_list(self):
        if self.select_employee_group:
            doc = frappe.get_doc("Duty Employee Group",
                                 self.select_employee_group)
            if doc:
    ...
    def set_se_items_finish(self, se):
        ...
        qty_of_total_production = float(
            qty_of_total_production) + float(self.total_stock)
        ...
        for data in cartons:
            se = self.set_se_items(se, data, None, se.from_warehouse, True, qty_of_total_production,
                                   total_sale_value, production_cost, raw_material=cartons[data])

        # add paper cup item to stockentry
        se = self.set_se_items(se, self.item, se.to_warehouse, None, True,
    ...
    def set_se_items(self, se, item, t_wh, f_wh, calc_basic_rate=False, qty_of_total_production=None, total_sale_value=None, production_cost=None, raw_material=None):
        ...
        if not expense_account and not item_expense_account:
            frappe.throw(
                "Please update default Default Cost of Goods Sold Account for company {0}".format(self.company))

        if not cost_center and not item_cost_center:
            frappe.throw(
                "Please update default Cost Center for company {0}".format(self.company))
