...
frappe.ui.form.on('Printing', {
	refresh: function(frm) {
		...
		if(!cur_frm.doc.__islocal){
			frm.set_query("item", "printing_inc_table", function () {
				return {
					filters: {"item_group": frm.doc.__onload.printingincitemgroup}
				}
			});
		}
	},
	setup: function (frm) {
		frm.set_query("workstation", function () {
			return {
				filters: {"paper_blank_machine_type": "Printing"}
			}
		});
		frm.set_query("reel_in", "printing_table", function () {
			return {
				filters: {"docstatus": 1, "status": "In Stock", "printed_reel": 0}
			}
		});
	},
