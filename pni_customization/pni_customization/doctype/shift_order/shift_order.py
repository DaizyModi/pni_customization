...
class ShiftOrder(Document):
	...
	def on_cancel(self):
		stock_entry = frappe.db.sql("""select name from `tabStock Entry`
			where pni_shift_order = %s and docstatus = 1""", self.name)
		if stock_entry:
	...
	def make_stock_entry(self, status):
		...
		stock_entry.pni_shift_order = self.name
		if status == "Submitted":
...
def manage_se_changes(doc, method):
	if doc.pni_shift_order:
		po = frappe.get_doc("Shift Order", doc.pni_shift_order)
		if(method=="on_submit"):
			if po.status == "Submitted":
