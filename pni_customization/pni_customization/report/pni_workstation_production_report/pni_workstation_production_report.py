	...
	return frappe.db.sql("""
		select table1.parent_item, table1.workstation_head, table1.machine_helper, table1.workstation,table1.total_production,table2.total_bottom_scrap, table2.total_blank_scrap
			from 
				...
				(select 
					item.variant_of as parent_item,packing.workstation_head as workstation_head, GROUP_CONCAT(DISTINCT(packing.machine_helper)) as machine_helper, packing.workstation as workstation, sum(pni_crt.total) as total_production 
				from 
				...
				where 
					...
					%s
				group by packing.workstation, packing.workstation_head) as table1
